...
TIVOS_FS_TARGET=$(readlink -f "${SCRIPTPATH}/../session/target")
TIVOS_TOOLS_BIN=$(readlink -f "${SCRIPTPATH}/../session/target/tools/bin")

# Cross compile an autotools package for our custom target (x86_64-tivos)
# Ensure the tivOS toolchain's in PATH
if [[ ":$PATH:" != *":$TIVOS_TOOLS_BIN:"* ]]; then
	export PATH=$TIVOS_TOOLS_BIN:$PATH
fi
...
if [ -z "$install_dir" ]; then
	install_dir=$(realpath -s "$TIVOS_FS_TARGET/usr/")
fi
