...
__NAME__="tiv-repo"
__DESCRIPTION__="The tivOS packaging system"
__VERSION__="0.0.1"
...
MSG="-- tiv-repo starting at $(date) --"

requirements() {
	if [[ "$(pwd)" =~ [^a-zA-Z0-9/_-] ]]; then
		echo "${RED}(x)${CLEAR} tiv-repo cannot be ran under directories that have non-alphanumeric characters in them! ($(pwd))"
		exit 1
	fi
...
closure() {
	echo -e "\n${GRAY}-- tiv-repo ended at $(date) --${CLEAR}"
}
...
upload_master() {
	if [ ! -f "out/master.lock.tiv" ]; then
		echo -e "${RED}(x)${CLEAR} Master lock could not be found!"
		exit 1
	fi

	utils/s3.sh "out/master.lock.tiv" "master.lock.tiv"
}
...
upload_single() {
	...
	source "pkgs/$1"
	ARCHIVE="out/${name}-${version}.tar.gz.tiv"
	METADATA="out/${name}-${version}.meta.tiv"
	...
	if [[ "$system" != "ignore" ]]; then
		utils/s3.sh "$ARCHIVE" "${name}-${version}.tar.gz.tiv"
	fi
	utils/s3.sh "$METADATA" "${name}-${version}.meta.tiv"
	upload_master
}
...
upload_all() {
	...
	# fire
	# uhhh, last chance, do we call it tiv or tivpkg?
	# tiv
